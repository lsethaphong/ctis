function prod = sps_matmul(fnameA,varA,fnameB,varB,fnameC,func,b_ki,b_kn,tol); %b_ki, b_kn is only if func is -1
...
row_cnt = 0;
ik_cnt = 0;
if func == 1 % A is signed column, row successive; B is signed row, column successive
    % summing vector
    tmp=zeros(A(1,2),1);
    idx_vec=zeros(1,1);
    c_ik=zeros(1,2);
    ...
    for i = 2:length(A)
        tmp(abs(A(i,2))) = A(i,1);
        if A(i,2) < 0 % reached end of row
            row_cnt = row_cnt +1;      
            for j = 2:length(B) % B is successive column
               sum  = sum + B(j,1)*tmp(abs(B(j,2)));
               if B(j,2) < 0 % reached end of column, now sum
                  col_cnt = col_cnt + 1;
                  if abs(sum) > tol
                     idx=idx+1;
                     c_ik(idx,1)= sum;
                     c_ik(idx,2)= col_cnt;
                     idx_z = 0;
                     % output was in tolerance
                  end
                  sum = 0;
               end
            end
            col_cnt =0;
            if idx_z > 0 % check if no entries higher than tolerance 
                idx = idx+1;
                c_ik(idx,1) = 0;
                c_ik(idx,2) = 1;
            end
            idx_z = 1; % reset zero watch
